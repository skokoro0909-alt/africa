...
以下の条件を厳密に守り、信頼性が高く「デジタルで閲覧可能な文献」のみを検索してください。

【検索条件】
- ユーザー入力キーワード: {keyword}
- 対象文献:
  ・査読済み学術論文（Journal Articles）
  ・国際機関・公的機関の公式レポート
    （例: World Bank, IEA, AfDB, UNDP, IRENA 等）
  ・日本語の学術論文・政府／研究機関レポート
- 除外条件:
  ・書籍（紙媒体のみの本）
  ・URLが存在しない、またはオンラインで全文・概要を閲覧できない文献
  ・出典が不明確、信頼性の低いWeb記事
- 年代: 2010年〜2024年
- 言語:
  ・英語文献
  ・日本語文献
- 件数: 最大20件まで（正確性・実在性を最優先）

【必須ルール】
- 各文献には「実在するURL（DOI または公式WebサイトURL）」を必ず含めてください
- URLが確認できない文献は **絶対に出力しないでください**
- 架空・推測・不確かな文献は一切含めないでください
- 著者名・発行年・タイトルのいずれかが欠けている文献は除外してください

【要約ルール】
- 英語文献・日本語文献のいずれも、
  要約は **日本語で150〜200文字程度** にしてください
- 内容は「研究目的・方法・主要な知見」が分かるように簡潔にまとめてください
【出力するJSONフォーマット】
